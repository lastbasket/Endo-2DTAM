...
def extract_mesh_bounded(scene_dir, data_config, voxel_size=0.1, sdf_trunc=0.5, depth_trunc=100, depth_scale=1, down_scale = 2,  mask_backgrond=True):
    ...
    o3d_intrinsics = []
    o3d_extrinsics = []
    cam_param['H'] = desired_height
    cam_param['W'] = desired_width
    ...
    cam_param['in'] = o3d_intrinsics
    cam_param['ex'] = o3d_extrinsics
...
if __name__ == '__main__':
    ...
    mesh, cam_param = extract_mesh_bounded(os.path.join(args.render, scene_name), data_config, down_scale=down_scale)
    name = 'fuse.ply'
    ...
    for c in range(len(cam_param['ex'])):
        if (count)%step == 0:
            # print(count)
            cameraLines = o3d.geometry.LineSet.create_camera_visualization(view_width_px=cam_param['W'], 
                                                                        view_height_px=cam_param['H'], 
                                                                        intrinsic=cam_param['in'][c], 
                                                                        extrinsic=cam_param['ex'][c])
    ...
    # poses_points = np.array([i[:3, 3] for i in cam_param['ex']])[::4]
    # line_index = np.array([[i, i+1] for i in range(poses_points.shape[0])])
