...
if __name__ == '__main__':
    ...
    color_render = os.path.join('data', dataset, scene_name, 'color')
    depth_render = os.path.join('data', dataset, scene_name, 'depth')
    
    
    extrinsics, eval_idx = read_pose_file_gt(pose_file)
    ...
    rgbmaps = np.array([cv2.resize(cv2.cvtColor(image, cv2.COLOR_BGR2RGB), (desired_width, desired_height)) for image in color2])[eval_idx]
    depthmaps = np.array([cv2.resize(np.array(Image.open(image_path)).astype(np.float32), (desired_width, desired_height)) 
                          / 2.55 for image_path in depth_files2])[eval_idx]
    ...
    name = 'fuse_traj.ply'
    save_dir = os.path.join(args.render, scene_name, 'eval', 'vis_recon_gt')
    if args.gray:
        save_dir = save_dir.replace('vis_recon', 'vis_recon_gt_gray')
