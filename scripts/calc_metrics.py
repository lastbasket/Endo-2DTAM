...
if __name__ == "__main__":
    ...
    if args.test_single:
        metrics = metric_single(os.path.join(args.gt, scene_name), os.path.join(args.render, scene_name))
        ...
        plot_3d_trajectories(metrics['traj'][0], metrics['traj'][1], metrics['pose'][0], os.path.join(args.render, scene_name, 'traj.png'))
        plot_traj_gt(metrics['traj'][0], os.path.join(args.render, scene_name, 'traj_gt.png'))
        ...
        with open(os.path.join(args.render, scene_name, 'results.csv'), 'w') as f:
            f.write('Sequence,PSNR,SSIM,LPIPS,RMSE,ATE\n')
            f.write(f"{scene_name},{results['PSNR']},{results['SSIM']},{results['LPIPS']},{results['RMSE']},{results['ATE']}\n")
            
            print("Results saved to: ", os.path.join(args.render, scene_name, 'results.csv'))
    else:
