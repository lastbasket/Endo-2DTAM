...
def report_progress(params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
    ...
    if i % every_i == 0 or i == 1:
        ...
        transformed_pts, _ = transform_to_frame(params, iter_time_idx, 
                                             gaussians_grad=False,
                                             camera_grad=False)

        # Initialize Render Variables
        rendervar = transformed_params2rendervar(params, transformed_pts)
        im, _, allmap = Renderer(raster_settings=data['cam'])(**rendervar)
        rastered_depth = allmap[0:1]
...
def eval_save(dataset, final_params, eval_dir, sil_thres, 
         ...
         exp_list=None, mode='train'):
    # timer = Timer()
    # timer.start()
    # use the total indexes
    split = False
    ...
    if save_renders:
        ...
        render_normal_dir = os.path.join(eval_dir, "normal")
    ...
    def get_idx(total_time_idx, mode='train'):
        if split:
    ...
    for total_time_idx in tqdm(range(num_frames)):
        ...
        dataset_type, dataset, time_idx = get_idx(total_time_idx, mode)
        ...
        exp_a = exp['exp_a'].detach()
        exp_b = exp['exp_b'].detach()
        ...
        #                 data_range=1.0, size_average=True, win_size=11)
        ...
        if save_renders:
            ...
            normal_save = render_normal.detach().cpu().numpy()
            normal_save = ((normal_save + 1)/2*255).astype(np.uint8)
            normal_save[..., 1] = -normal_save[..., 1]
            normal_save[..., 2] = -normal_save[..., 2]
            normal_save = cv2.cvtColor(normal_save, cv2.COLOR_RGB2BGR)
            cv2.imwrite(os.path.join(render_normal_dir, "normal_{:04d}.png".format(total_time_idx)), normal_save)
    ...
    if split:
        with open(os.path.join(eval_dir, 'gt_train_w2c.txt'), 'w') as f:
            for tensor in [gt_w2c_list[idx] for idx in train_idx]:
                valid_gt_w2c_list.append(tensor)
                line = ''
        ...
        with open(os.path.join(eval_dir, 'est_w2c.txt'), 'w') as f:
            for tensor in latest_est_w2c_list:
                ...
                for v in tensor.reshape(-1).cpu().numpy():
                    line += str(v) + ','
                f.write(line[:-1] + '\n')
        torch.save(exp_list, os.path.join(eval_dir, 'exp.ckpt'))
    # timer.end()
    gt_w2c_list = valid_gt_w2c_list
    # print(len(gt_w2c_list))
    # print(len(latest_est_w2c_list))
    # exit()
    # # Calculate ATE RMSE
    ate_rmse = evaluate_ate(gt_w2c_list, latest_est_w2c_list)
    print("Final Average ATE RMSE: {:.2f} mm".format(ate_rmse))
    ...
    print("Average Depth RMSE: {:.2f} mm".format(avg_rmse))
    print("Average Depth L1: {:.2f} mm".format(avg_l1))
